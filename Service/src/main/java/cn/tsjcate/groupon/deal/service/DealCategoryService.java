...
import cn.tsjcate.groupon.deal.cache.DealCategoryCacheOperator;
import cn.tsjcate.groupon.deal.constant.DealConstant;
import cn.tsjcate.groupon.deal.dao.DealCategoryDAO;
...
//        CacheUtil.getAllEntities(DealCategory.class);
        if (allCategories == null || allCategories.size() == 0) {
            ...
            for (DealCategory dealCategory : allCategories) {
                dealCategoryCacheOperator.putDealCategory(dealCategory);//将分类信息放入redis缓存中
...
//        dealCategoryCacheOperator.deleteDealCategory(categoryId);
    ...
    public List<DealCategory> getLaterGenerationCategories(Long categoryId) {
        ...
        List<DealCategory> subCategories = getSubCategories(categoryId);//获得子类别
