package cn.tsjcate.groupon.order.service;

import cn.tsjcate.groupon.order.dao.OrderDAO;
import cn.tsjcate.user.service.UserService;
import cn.tsjcate.framework.base.exception.BusinessException;
import cn.tsjcate.framework.common.Pair;
import cn.tsjcate.framework.common.page.PagingResult;
import cn.tsjcate.framework.common.search.Search;
import cn.tsjcate.groupon.cart.entity.Cart;
...
import cn.tsjcate.groupon.order.entity.OrderDetail;
import cn.tsjcate.support.address.entity.Address;
...
//        details.stream().collect(Collectors.toMap(OrderDetail::getId, detail -> detail));
    ...
    public PagingResult<Order> findOrderForPage(Search search) {
        PagingResult<Order> result = this.orderDAO.findPage(search);//根据条件，以及分页信息的到所要的订单信息
