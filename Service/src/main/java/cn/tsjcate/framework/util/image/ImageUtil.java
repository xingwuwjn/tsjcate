...
import cn.tsjcate.framework.base.exception.FrameworkException;
...
public class ImageUtil {
	...
	public static void generateImage(String moduleName, Long imgId, String srcPath) throws Exception {
		List<Integer> imageIndexList = imageConfig.getImageIndexList(moduleName);//获得某个module下的所有大小的key
		if (imageIndexList != null && imageIndexList.size() > 0) {
			for (Integer imageIndex : imageIndexList) {//循环遍历指定module所有设定大小，构造图片
				//构建新的路径
				...
				doGenerateImage(moduleName, imageIndex, srcPath, newPath, "75");//裁剪图片
			}
		}
	}
	...
	public static void generateDetailImage(String moduleName, Long imgId, String srcPath) throws Exception {
		...
		if (imageIndexList != null && imageIndexList.size() > 0) {
			for (Integer imageIndex : imageIndexList) {//遍历生成每个module下所有大小的图片
				String newPath = imageConfig.getDetailDestinationBasePath() +
						getDetailImageGenerateFileRelativePath(imgId, moduleName, imageIndex);
	...
	private static String doGenerateImage(String moduleName, Integer imageIndex, String srcPath,
		...
		String imageSize = imageConfig.getImageSize(moduleName, imageIndex);//获得图片尺寸
		...
		GraphicsTools.cutImage(width, height, srcPath, newPath, quality);//执行裁剪图片
	...
	public static String getImageGenerateFileRelativePath(Long imgId, String moduleName, Integer imageIndex) {
		...
		path.append(toHex32(XOR(imgId / 1000000))).append(GlobalConstant.PATHSEPERATOR)
				.append(toHex32(XOR(imgId.intValue() % 1000000 / 100)))
