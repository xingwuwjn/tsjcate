...
function searchData(gridId) {
	var id = '#' + gridId;
    var params = $(id).datagrid('options').queryParams;//数据框自定义router参数
	var fields = $('#searchForm').serializeArray();//序列化所有的表单数据
...
function processAjaxCallResult(result, gridId, dialogId) {
	...
	if (typeof result == "string") {//如果为字符串则将json字符串转换为json对象
		data = eval('('+result+')');
	} else {
		data = result;
	}
	
	if (data['statusCode'] == '0' || data['statusCode'] == 0) {//操作成功
		if (dialogId) {
			$('#' + dialogId).dialog('close');
		}
		if (gridId) {
			$('#' + gridId).datagrid('reload');//刷新数据面板
			$.messager.show({
				title: '提示',
				msg: '操作成功',
				style:{
        ...
        return data['message'];
	} else if (data['statusCode'] == '1' || data['statusCode'] == 1) {//警告
		msg = data['message'];
	} else if (data['statusCode'] == '2' || data['statusCode'] == 2) {//失败
		msg = data['message'];
	} else {
		msg = '操作中出现异常状况';
	}
...
var Messager = {
	...
	Prompt: function(title, msg) {
		$.messager.show({
	        ...
	        style:{
	            right:'',
	            top:document.body.scrollTop+document.documentElement.scrollTop,//获取滚动条的位置，两个有一个有值，一个无值
	            bottom:''
	        }
	    });
	},
...
var Submit = {
	FormSubmit: function(formId, dialogId, gridId, url, callBackFunction, beforeSubmitFunction) {
		$('#' + formId).form('submit',{
            url: url,
            onSubmit: function(){//点击提交按钮就执行的方法
            	if (beforeSubmitFunction) {
            		beforeSubmitFunction();
            	}
                return $(this).form('validate');//刷新表单
            },
            success: function(result){
            	processAjaxCallResult(result, gridId, dialogId);//请求后处理
            	if (callBackFunction) {
            		callBackFunction();
            	}
            }
	...
	FormDeleteSubmit: function(formId, gridId, url, callBackFunction) {
		var row = $('#' + gridId).datagrid('getSelected');//返回第一个被选中的行或如果没有选中的行则返回 null。
    	if (!row) {
    		Messager.Prompt('提示', '请选择要删除的记录。');
            return;
    	}
    	$('#' + formId).form('load',row);//填充表单
    	$.messager.confirm('确认', '确定要删除该记录?', function(data){
            if (data) {
               $('#' + formId).form('submit',{
	                url: url,
	                onSubmit: function(){
	                    return $(this).form('validate');//做表单字段验证所有有效返回true
	                },
	                success: function(result){
	                	processAjaxCallResult(result, gridId, null);//请求后处理
	                	if (callBackFunction) {
	                		callBackFunction();
	                	}
	                }
